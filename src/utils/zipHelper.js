  ...
  static async validate(zipPath) {
    try {
      ...
      if (dangerousFiles.length > 0) {
        const flagged = dangerousFiles.map(f => f.path).join(', ');
        logger.warn(`ZIP rejected — dangerous files found: ${flagged}`);
        return {
          valid: false,
          error: `Security Alert: ZIP contains potentially dangerous files. Please remove these files and upload again. Flagged files: ${flagged}`,
