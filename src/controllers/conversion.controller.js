...
export const startConversion = asyncHandler(async (req, res) => {
  ...
  // Enhanced async handling with better error management
  res.status(202).json({
    success: true,
    data: {
      job: {
  ...
  // Start conversion asynchronously with better error handling
  ConversionService.startConversion(
    ...
    .then(result => {
      ...
      logger.info(`Conversion job ${job.id} completed successfully and broadcasted`);
    })
    .catch(error => {
      // Broadcast failure
