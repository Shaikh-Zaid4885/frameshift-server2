...
// Cache jobId -> userId to avoid DB lookups on every progress tick
const jobUserIdCache = new Map();
...
export const broadcastProgress = async (jobId, progressData) => {
  try {
    let userId = jobUserIdCache.get(jobId);

    if (!userId) {
      // Only hit the DB when we haven't cached this job yet
      ...
      userId = job.user_id;
      jobUserIdCache.set(jobId, userId);

      // Auto-evict after 35 minutes (slightly longer than the 30-min conversion timeout)
      setTimeout(() => jobUserIdCache.delete(jobId), 35 * 60 * 1000);
    }
    ...
    const message = {
      ...
      conversion_mode: progressData.conversion_mode || 'default',
      timestamp: Date.now()
    };
...
/**
 * Broadcast generated diff to user in real-time (streaming)
 * @param {string} jobId - Conversion job ID
 * @param {Object} diffData - Diff data for the file
 */
export const broadcastDiffGenerated = async (jobId, diffData) => {
  try {
    ...
    if (!job) {
      logger.warn(`Job not found for diff broadcast: ${jobId}`);
      return;
    }
    ...
    const message = {
      type: 'conversion:diff_generated',
      jobId,
      diff: diffData,
      timestamp: Date.now()
    };
    ...
    broadcastToUser(userId, message);
    logger.debug(`Broadcasted diff for job ${jobId} to user ${userId}`);
  } catch (error) {
    logger.error(`Failed to broadcast diff for job ${jobId}:`, error);
  }
};
