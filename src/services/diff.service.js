...
class DiffService {
  ...
  /**
   * Django-specific patterns that indicate a line needs manual attention
   * if it survived into the converted Flask output.
   */
  static DJANGO_PATTERNS = [
    { pattern: /from django[\s.]/, reason: 'Django import — replace with Flask equivalent' },
    { pattern: /import django/, reason: 'Django import — replace with Flask equivalent' },
    { pattern: /\.objects\./, reason: 'Django ORM call — convert to SQLAlchemy query' },
    { pattern: /{%\s*(url|load|csrf_token|static|block|extends|include)\b/, reason: 'Django template tag — convert to Jinja2 syntax' },
    { pattern: /\{\{\s*request\./, reason: 'Django template variable — review for Flask compatibility' },
    { pattern: /AUTH_USER_MODEL/, reason: 'Django AUTH_USER_MODEL reference — use Flask-Login UserMixin' },
    { pattern: /INSTALLED_APPS/, reason: 'Django settings reference — not used in Flask' },
    { pattern: /MIDDLEWARE/, reason: 'Django middleware reference — use Flask middleware/decorators' },
    { pattern: /urlpatterns/, reason: 'Django URL config — use Flask route decorators or blueprints' },
    { pattern: /from rest_framework/, reason: 'Django REST Framework import — use Flask-RESTful or similar' },
    { pattern: /serializers\./, reason: 'DRF serializer — replace with marshmallow or manual serialization' },
    { pattern: /class Meta:/, reason: 'Django Meta class — review for SQLAlchemy equivalent (__tablename__, etc.)' },
    { pattern: /HttpResponse\(|HttpResponseRedirect\(/, reason: 'Django response — use Flask make_response/redirect' },
    { pattern: /get_object_or_404/, reason: 'Django shortcut — use SQLAlchemy .query.get_or_404()' },
    { pattern: /render_to_string/, reason: 'Django render_to_string — use Jinja2 render_template_string' },
    { pattern: /django\.conf\.settings/, reason: 'Django settings — use Flask app.config' },
    { pattern: /\.values\(|\.values_list\(/, reason: 'Django QuerySet method — convert to SQLAlchemy equivalent' },
    { pattern: /forms\.\w+Field/, reason: 'Django form field — convert to WTForms field' },
    { pattern: /(?:#|<!--|{#)\s*TODO:/, reason: 'Converter left a TODO — requires manual implementation' },
    { pattern: /django\.contrib\.admin/, reason: 'Django admin — Flask has no built-in admin; use Flask-Admin' },
    { pattern: /django\.core\.management/, reason: 'Django management command — use Flask CLI (click)' },
    { pattern: /django\.dispatch|Signal\(/, reason: 'Django signals — use blinker or Flask signals' },
    { pattern: /django\.core\.mail/, reason: 'Django mail — use Flask-Mail instead' },
    { pattern: /django\.contrib\.auth/, reason: 'Django auth — use Flask-Login / Flask-Security' },
    { pattern: /messages\.\w+\(/, reason: 'Django messages framework — use Flask flash() instead' },
  ];

  /**
   * Find unchanged Django-specific lines in the converted content
   * These are lines that still contain Django patterns and need manual attention
   * @param {Object} diffData - Diff data object with content.converted
   * @returns {Array} Array of {line, lineNumber, reason, code}
   */
  findUnchangedDjangoLines(diffData) {
    const results = [];
    const convertedContent = diffData?.content?.converted;

    if (!convertedContent) return results;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Skip empty lines (and comments, unless they contain TODO)
      if (!trimmed || (trimmed.startsWith('#') && !trimmed.toLowerCase().includes('todo'))) continue;

      for (const { pattern, reason } of DiffService.DJANGO_PATTERNS) {
